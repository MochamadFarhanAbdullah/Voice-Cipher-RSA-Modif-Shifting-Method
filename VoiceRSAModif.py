...
def main():
    def Shift_Left(binary, shift):
        return binary[shift:] + binary[:shift-1]

    def Shift_Right(binary, shift):
    ...
    if choice == "Tutorial":
        ...
        st.write(
            "a. Buat kunci dengan memilih page Make Keys dan catat hasilnya karena nanti akan digunakan"
        )
        st.write(
            "b. Jika tidak tahu kuncinya maka bisa memilih page Custom Prime, catat hasilnya untuk digunakan di page Make Keys"
        )
        ...
        elif choice == "Membuat file dari teks base64":
        ...
        def encryption_menu():
            ...
            shift = st.number_input(
                "Masukkan nilai shift untuk penggeseran (shift):", min_value=1, step=1, value=1, key="encryption_shift"
            )
                ...
                if process_path.strip() != "":
                    ...
                    with open(process_path, "a") as base64_file:
                        ...
                        for i in range(0, len(message)):
                            ...
                            bit_before = bin(encrypted_block)[2:].zfill(8)
                            bit_after = Shift_Left(bit_before, shift)
                            ...
                            base64_file.write(
                                f"Block '{block}' is {encoded_block}, {encoded_block}^{e} mod {n} is {encrypted_block}, bit before is {bit_before}, shifted to {bit_after}\n"
                            )
        ...
        def decryption_menu(max_digit, shift):
                ...
                if process_path.strip() != "":
                    ...
                    with open(process_path, "a") as base64_file:
                        ...
                        for block in ciphertext_blocks:
                            ...
                            st.write(block)
                            bit_after = bin(decrypted_block)[2:].zfill(8)
                            bit_before = Shift_Right(bit_after, shift)
                            ...
                            # Write the details of the current block to the file
                            base64_file.write(
                                f"{block}^{d} mod {n} is {decrypted_block}, shifted right to {bit_after}, shifted back to {bit_before}\n"
                            )
        ...
        if choice == "Make keys":
            ...
            shift = st.number_input(
                "Enter the shift value:", value=1, min_value=1, step=1
            )
            decryption_menu(max_digit, shift)
